...
GLmol.prototype.parsePDB2 = function(str) {
   ...
   for (var i = 0; i < lines.length; i++) {
      ...
      if (recordName == 'ATOM  ' || recordName == 'HETATM') {
         var atom, resn, chain, resi, x, y, z, hetflag, elem, serial, altLoc, b, check;
		 ...
		 //HERE ADDED recordName AT THE END, SO THAT WE CAN COLOR THEM according to chains with only atoms for recordName
         atoms[serial] = {'resn': resn, 'x': x, 'y': y, 'z': z, 'elem': elem,
  'hetflag': hetflag, 'chain': chain, 'resi': resi, 'serial': serial, 'atom': atom,
  'bonds': [], 'ss': 'c', 'color': 0xFFFFFF, 'bonds': [], 'bondOrder': [], 'b': b , 'recordName': recordName /*', altLoc': altLoc*/};
...
GLmol.prototype.drawSmoothCurve = function(group, _points, width, colors, div) {
   ...
   for (var i = 0; i < points.length; i++) {
	  ...
	  //WAS NOT COMMENTED OUT ORIGINALLY, IT MAKES THE CARTOON LOOK LIKE ANIME IF COMMENTED OUT.
...
GLmol.prototype.colorByAtom = function(atomlist, colors) {
   ...
   for (var i in atomlist) {
	  ...
	  //BU TAM ASAGIDAKI LINE, ELEMENTE GORE RENKLENDIRIR, MESELA BIZIM SECILMEYEN CHAINLER HEP GREYDI, MEGERSE CARBON RENGI IMIS: 0xAAAAAA
	  //AMA NUCLEIC ACID OLUNCA OKSIJEN RENGINI ALMIS O DA KIRMIZI, BU KISMI COMMENT OUT ETTIK, defaultColor kismi ayni kaldi ama bu renk degisti, carbon grisine, instead of cccccc

	  //comment out etmessen, element colorlari default renkli oluyor, comment olursa hepsi gri
...
//GLmol.prototype.colorByChain = function(atomlist, colors) {

	//I MADE SOME CHANGES TO UNDERSTAND IF THE STRUCTURE HAS MORE THAN ONE CHAIN. IF ONE CHAIN, THEN JUST COLOR BY GREY
//	var number=this.myChains.length;
	var number=0;
	var temp;
	...
	var names = [];
	    ...
	    for (var i in atomlist) {
  			...
  			//alert(atom.recordName+"sannen");

  			if(i==0){ names[0]=atom.chain; number++;}

  			else{
  				// if(atom.recordName == 'ATOM  '){
//   					temp = this.atoms[atomlist[i-1]];
//   					if(temp.recordName == 'ATOM  '){
//   						if(atom.chain != temp.chain){number++; }
//   					}
//   				}

  				temp = this.atoms[atomlist[i-1]];

  				if(atom.chain != temp.chain){names[number]=atom.chain; number++; }


  			}

  	   }
	  	   //  CAN BE USED LATER FOR REMOVING DUPLICATE CHAINS, FOR NOW FINE
	   var uniqueNames = [];
	   $.each(names, function(i, el){
	       if($.inArray(el, uniqueNames) === -1) uniqueNames.push(el);
	   });
  	   //alert(number+"\t"+uniqueNames.length);

	   number = uniqueNames.length;

	 //  alert(number+"\t"+uniqueNames.length);

	   //BURAYI DEGIS SONRA NOW THAT WE HAVE CHAINS INFO!
	   ...
	   if(number==1){
                  ...
                  atom.color = 0x999999;
	   ...
	   else if(number==2 && this.myChains.length ==0){
  	  	        ...
  	  	        for (var i in atomlist) {
  				    ...
  				    for (var y=0; y < uniqueNames.length; y++ ){
  							...
  							if(atom.chain == uniqueNames[0]){
  								//alert(atom.chain+"\t"+ this.myChains[y]);

  								atom.color = 0x999999;

  							}

  							else if (atom.chain == uniqueNames[0]){
		   ...
		   // if (this.myChains.length == undefined && number == 2){
//
// 	  	        for (var i in atomlist) {
//
// 	  	       	 	var atom = this.atoms[atomlist[i]]; if (atom == undefined) continue;
...
// 			else{
		   ...
		   for (var y=0; y < this.colorsAll.length; y++ ){

			   	 if(this.colorsAll[y].indexOf('#') > -1){

					  this.colorsAll[y] = this.colorsAll[y].replace("#","0x");

					  //alert(this.colorsAll[y]);
		        ...
		        for (var i in atomlist) {
			      ...
			      if (colorSidechains || atom.atom == 'CA' || atom.atom == 'O3\'') {
					    ...
					    for (var y=0; y < this.myChains.length-1; y++ ){

		 						if(atom.chain == this.myChains[y]){
		 							//alert(atom.chain+"\t"+ this.myChains[y]);
		 							atom.color = this.colorsAll[y];
...
GLmol.prototype.createTextTex = function(text, size, color) {
   ...
   //SOLVES PROBLEM WITH EXTRA PRINTING OF THE LABEL AT THE BOTTOM OF THE PAGE
...
GLmol.prototype.defineRepresentation = function() {
   ...
   //CHANGED hetatm to [] to not to show het atoms on first initialization of the view
   this.drawAtomsAsSphere(this.modelGroup, [], this.sphereRadius);

   //THIS ONE WAS NOT COMMENTED OUT BEFORE, BUT IS NOT NEEDED BECAUSE WHEN THERE IS A NUCLEIC ACID, IT LOOKS PURPLE AND LIKE A RIBBON
   //this.drawMainchainCurve(this.modelGroup, all, this.curveWidth, 'P');


   //INSTEAD WE ADDED THE FOLLOWING FUNCTION WHICH WAS CREATED BEFORE, JUST CALLED IT NOW
...
GLmol.prototype.rebuildScene = function() {
  ...
  // alert("builded scene in " + (+new Date() - time) + "ms");
...
GLmol.prototype.loadMoleculeStr = function(repressZoom, source, colors, chains) {
...
GLmol.prototype.enableMouse = function() {
   ...
   $('body').bind('mouseup touchend', function(ev) {
      ...
      // console.log(atom.chain+"\t"+atom.resn+"\t"+atom.resi);

      //CHANGED BY MELIS TO GET INFORMATION ON THE RESIDUE BEING CLICKED ON THE STRUCTURE TO THE URL, THEN THIS WILL BE LATER USED FOR OTHER FUNCTIONS
      viewOptions.residue = atom.resn + ":" + atom.resi;
      updateURL(viewOptions);

	  //MELIS VERSION
	  var bb = me.billboard(me.createTextTex(atom.chain + ":" + atom.resn + ":" + atom.resi, "80", "#428bca"));
